...
client = pymongo.MongoClient("mongodb+srv://Sriee:0112@cluster0.7isgtih.mongodb.net/?retryWrites=true&w=majority")
twtdb = client.project
twtscr = twtdb.twtscrdb
...
def main():
  ...
  # Menu 3 is a search option
    ...
    if submit_button:
      ...
      for tweet in enumerate(twitterscraper.TwitterSearchScraper(f"from:{query} since:{start} until:{end}").get_items()):
        ...
        # Stores the tweet data into MongoDB until the limit  is reached
          ...
          twtscr.insert_one(new)
          tweets += 1
          print(tweet.statuscode)
      # Display the total tweets scraped
      df = pd.DataFrame(list(twtscr.find()))
      cnt = len(df)
